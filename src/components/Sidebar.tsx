...
const Sidebar = ({
  ...
  const homeContent = <div className="p-6 bg-white rounded-lg shadow">
  ...
  const historyContent = <div className="p-6 bg-white rounded-lg shadow">
      ...
      {conversations.length > 0 ? <>
          ...
          <ul className="space-y-2 text-gray-600">
            {conversations.map(conv => <li key={conv.id} className="p-2 bg-gray-50 rounded">
  ...
  const feedbackContent = <div className="p-6 bg-white rounded-lg shadow">
  ...
  const settingsContent = <div className="p-6 bg-white rounded-lg shadow">
      ...
      <div className="space-y-4">
        <div className={`p-3 border rounded-lg cursor-pointer transition-all ${selectedModel === 'coach' ? 'border-mitra-sky-blue bg-blue-50' : 'border-gray-200 hover:border-mitra-light-blue'}`} onClick={() => setSelectedModel('coach')}>
        ...
        <div className={`p-3 border rounded-lg cursor-pointer transition-all ${selectedModel === 'cryBuddy' ? 'border-mitra-sky-blue bg-blue-50' : 'border-gray-200 hover:border-mitra-light-blue'}`} onClick={() => setSelectedModel('cryBuddy')}>
        ...
        <div className={`p-3 border rounded-lg cursor-pointer transition-all ${selectedModel === 'mindReader' ? 'border-mitra-sky-blue bg-blue-50' : 'border-gray-200 hover:border-mitra-light-blue'}`} onClick={() => setSelectedModel('mindReader')}>
  ...
  return <>
      ...
      {!isOpen && <div className="fixed left-0 top-4 z-20">
          <Button variant="ghost" size="icon" onClick={toggleSidebar} className="bg-mitra-light-blue hover:bg-blue-200 ml-2">
      ...
      <aside className={`bg-mitra-light-blue transition-all duration-300 ${isOpen ? 'w-64' : 'w-0 overflow-hidden'} min-h-screen flex flex-col`}>
        ...
        <nav className="flex-1 px-4">
          <ul className="space-y-2">
            <li>
              <Button variant={activeTab === "home" ? "default" : "ghost"} className={`w-full justify-start gap-3 hover:bg-blue-200 ${activeTab === "home" ? "bg-blue-300" : ""}`} onClick={() => handleTabClick("home")}>
                <Home size={20} />
                <span className="ml-2">Home</span>
              </Button>
            </li>
            <li>
              <Button variant={activeTab === "history" ? "default" : "ghost"} className={`w-full justify-start gap-3 hover:bg-blue-200 ${activeTab === "history" ? "bg-blue-300" : ""}`} onClick={() => handleTabClick("history")}>
                <Clock size={20} />
                <span className="ml-2">Session History</span>
              </Button>
            </li>
            <li>
              <Button variant={activeTab === "feedback" ? "default" : "ghost"} className={`w-full justify-start gap-3 hover:bg-blue-200 ${activeTab === "feedback" ? "bg-blue-300" : ""}`} onClick={() => handleTabClick("feedback")}>
        ...
        <div className="mt-auto p-4 py-[28px] px-[15px]">
          {/* Voice Gender Selector - moved here before settings */}
          <div className="mb-3 flex justify-center py-0 my-0 mx-[60px] px-0">
            <VoiceGenderSelector selectedGender={voiceGender} onGenderChange={handleVoiceGenderChange} />
          </div>
        
          <Button variant={activeTab === "settings" ? "default" : "ghost"} className={`w-full justify-start gap-3 hover:bg-blue-200 ${activeTab === "settings" ? "bg-blue-300" : ""}`} onClick={() => handleTabClick("settings")}>
            <Settings size={20} />
            <span className="ml-2">Settings</span>
          </Button>
        </div>

        {activeTab && <div className="px-4 pb-4">
